...
def generate_random_numbers(start: int, end: int, count: int, excluded_values: [] = None) -> []:
...
def main():
    ...
    df["artist_ids"] = df.apply(
        lambda x: json.dumps(generate_random_numbers(1, NUMBER_OF_ALBUMS, random.randint(2, 7))),
        axis=1)

    # Fill friends with numbers between 5 and 15
    df["friends"] = df.apply(
        lambda x: json.dumps(generate_random_numbers(1, NUMBER_OF_USERS, random.randint(5, 15), [x.iloc[0]])),
        axis=1)
